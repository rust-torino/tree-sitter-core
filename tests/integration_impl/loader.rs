...
use crate::integration_impl::{
    bindings::Language, tree_sitter_highlight::HighlightConfiguration,
    util::get_treesitter_repository,
};
...
static BUILD_TARGET: Lazy<String> = Lazy::new(|| {
    let cc = match which("gcc") {
        Ok(path) => path,
        Err(_) => match which("clang") {
            Ok(path) => path,
            Err(_) => panic!("unable to find a compatible C compiler to get host triple"),
        },
    };

    let stdout = Command::new(cc)
        .arg("-dumpmachine")
        .output()
        .expect("unable to run a C compiler in order to get host triple")
        .stdout;
...
impl Loader {
    ...
    pub fn load_language_from_sources(
        ...
        Self::put_headers_into(header_path)?;
    ...
    fn put_headers_into(header_path: &Path) -> Result<()> {
        let repo_path = get_treesitter_repository()?;

        const HEADERS: [&str; 2] = ["parser.h", "api.h"];
        let tree_sitter_include_path = header_path.join("tree_sitter");

        if tree_sitter_include_path.exists() {
            assert!(tree_sitter_include_path.is_dir());
        } else {
            let repo_include_path = repo_path.join("lib").join("include").join("tree_sitter");
            assert!(repo_include_path.exists());

            fs::create_dir_all(&tree_sitter_include_path)?;

            for header_name in &HEADERS {
                let header_src = repo_include_path.join(header_name);
                assert!(header_src.exists());

                fs::copy(&header_src, &tree_sitter_include_path.join(header_name))?;
            }
        }
